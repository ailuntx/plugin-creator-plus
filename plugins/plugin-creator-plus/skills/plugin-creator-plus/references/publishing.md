...
When a task actually needs stored credentials, read the installed Keybook skill and use its `doctor`, name search, and `run --env … -- <trusted consumer>` flow. Review the consumer before injection; it must use the selected variable. Keybook suppresses child output, so persist only deliberately selected, nonsecret outcomes if needed. Never read the YAML into context, try unrelated password variants, extract browser session tokens, or package credentials. An existing browser login normally needs no credential lookup.
